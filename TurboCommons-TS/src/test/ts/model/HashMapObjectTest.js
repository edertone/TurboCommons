 ...
 * testSortByValue
 */
QUnit.test("testSortByValue", function(assert){
    
    // Test empty values
    for (var i = 0; i < window.emptyValuesCount; i++) {

        assert.throws(function() {
            window.populatedHashMap.sortByValue(window.emptyValues[i]);
        });

        assert.throws(function() {
            window.populatedHashMap.sortByValue(HashMapObject.SORT_METHOD_NUMERIC, window.emptyValues[i]);
        });
    }

    var h = new HashMapObject();
    assert.ok(h.length() === 0);
    assert.ok(h.sortByValue());
    assert.ok(h.length() === 0);
    ...
    h.set('0', 'b');
    assert.ok(h.length() === 5);
    assert.ok(h.sortByValue(HashMapObject.SORT_METHOD_STRING));
    assert.ok(h.length() === 5);
    assert.ok(ArrayUtils.isEqualTo(h.getKeys(), ['b', '0', 'a', 'c', 'd']));
    assert.ok(h.get('0') === 'b');

    assert.ok(h.sortByValue(HashMapObject.SORT_METHOD_STRING, HashMapObject.SORT_ORDER_DESCENDING));
    assert.ok(ArrayUtils.isEqualTo(h.getKeys(), ['d', 'c', 'a', '0', 'b']));
    ...
    assert.ok(h.sortByValue(HashMapObject.SORT_METHOD_STRING));
    assert.ok(h.length() === 5);
    assert.ok(ArrayUtils.isEqualTo(h.getKeys(), ['c', 'd', '0', 'a', 'b']));
    assert.ok(h.sortByValue(HashMapObject.SORT_METHOD_NUMERIC));
    assert.ok(h.length() === 5);
    assert.ok(ArrayUtils.isEqualTo(h.getKeys(), ['d', '0', 'a', 'b', 'c']));

    assert.ok(h.sortByValue(HashMapObject.SORT_METHOD_STRING, HashMapObject.SORT_ORDER_DESCENDING));
    assert.ok(ArrayUtils.isEqualTo(h.getKeys(), ['b', 'a', '0', 'd', 'c']));

    assert.ok(h.sortByValue(HashMapObject.SORT_METHOD_NUMERIC, HashMapObject.SORT_ORDER_DESCENDING));
    assert.ok(ArrayUtils.isEqualTo(h.getKeys(), ['c', 'b', 'a', '0', 'd']));
    assert.ok(h.length() === 5);
