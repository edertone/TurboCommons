...
export class BrowserManager {
  ...
  scrollTo(
    ...
    const destinationValueX = ArrayUtils.isArray(destination)
      ? (<[number, number]>destination)[0]
      : (<HTMLScriptElement>destination).offsetLeft;
    const destinationValueY = ArrayUtils.isArray(destination)
      ? (<[number, number]>destination)[1]
      : (<HTMLScriptElement>destination).offsetTop;
    ...
    if (documentWidth > windowWidth) {
      destinationX = Math.round(
        documentWidth - destinationValueX < windowWidth
    ...
    if (documentHeight > windowHeight) {
      destinationY = Math.round(
        documentHeight - destinationValueY < windowHeight
    ...
    if ("requestAnimationFrame" in window === false) {
      (window as Window & typeof globalThis).scroll(destinationX, destinationY);
    ...
    function animate() {
      ...
      const x = Math.ceil(
        easingFunction(time) * (destinationX - startX) + startX,
      );
      const y = Math.ceil(
        easingFunction(time) * (destinationY - startY) + startY,
      );

      window.scroll(x, y);

      if (
        Math.ceil(window.pageXOffset) === destinationX &&
        Math.ceil(window.pageYOffset) === destinationY
      ) {
        if (callback !== null) {
          callback();
        }
  ...
  browseLocalFiles(
    event: any,
    mode: "TEXT" | "BASE64",
    callback: (fileNames: string[], fileContents: string[]) => void,
  ) {
