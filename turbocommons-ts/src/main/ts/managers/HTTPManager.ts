...
export class HTTPManager {
  ...
  generateUrlQueryString(
    keyValuePairs: { [key: string]: any } | HashMapObject,
  ) {
    let result = "";
    let keys: string[] = [];
    let values: string[] = [];
    ...
    if (
      ObjectUtils.isObject(keyValuePairs) &&
      ObjectUtils.getKeys(keyValuePairs).length > 0
    ) {
  ...
  isInternetAvailable(yesCallback: () => void, noCallback: () => void) {
    ...
    let recursiveUrlTest = (urls: string[]) => {
      ...
      this.urlExists(
        String(url + "?r=" + StringUtils.generateRandom(15, 15)),
        yesCallback,
  ...
  getUrlHeaders(
    ...
    xmlHttprequest.onload = () =>
      successCallback(xmlHttprequest.getAllResponseHeaders().split("\n"));

    xmlHttprequest.onerror = () =>
      errorCallback(xmlHttprequest.statusText, xmlHttprequest.status);
  ...
  execute(
    ...
    progressCallback:
      | null
      | ((completedUrl: string, totalRequests: number) => void) = null,
  ) {
    let requestsList = this._generateValidRequestsList(requests);

    // Validate callbacks are ok
    if (
      (finishedCallback !== null && !(finishedCallback instanceof Function)) ||
    ...
    const processFinishedRequest = (
      requestWithIndex: { index: number; request: HTTPManagerBaseRequest },
      response: string,
      isError: boolean,
      errorMsg: string,
      code: number,
      ...
      finishedCount++;
      finishedResults[requestWithIndex.index] = {
        url: composedUrl,
    ...
    for (let i = 0; i < requestsList.length; i++) {
      ...
      if (
        !StringUtils.isString(requestsList[i].url) ||
        StringUtils.isEmpty(requestsList[i].url)
      ) {
      ...
      try {
        ...
        throw new Error(
          "Could not initialize XMLHttpRequest. If running node, it is not natively available. We recommend npm xhr2 library that emulates XMLHttpRequest on node apps (global.XMLHttpRequest = require('xhr2'))",
        );
      }
      ...
      if (requestsList[i].timeout > 0 || this.timeout > 0) {
        xmlHttprequest.timeout =
          requestsList[i].timeout > 0 ? requestsList[i].timeout : this.timeout;
      }

      // Detect the request type
      let composedUrl = this._composeUrl(this.baseUrl, requestsList[i].url);
      let requestType =
        requestsList[i] instanceof HTTPManagerGetRequest ? "GET" : "POST";
      ...
      xmlHttprequest.ontimeout = () => {
        processFinishedRequest(
          ...
          xmlHttprequest.responseText,
          true,
          this.timeout + HTTPManager.ERROR_TIMEOUT,
          408,
        );
      };
      ...
      if (
        requestType === "POST" ||
        Object.keys(this._globalPostParams).length > 0
      ) {
        try {
          let postParamsToSend =
            requestType === "POST"
              ? (requestsList[i] as HTTPManagerPostRequest).parameters
              : {};
          ...
          if (requestsList[i].ignoreGlobalPostParams === false) {
            for (let globalPostParam of Object.keys(this._globalPostParams)) {
              if (postParamsToSend instanceof HashMapObject) {
                postParamsToSend.set(
                  globalPostParam,
                  this._globalPostParams[globalPostParam],
                );
              } else {
                postParamsToSend[globalPostParam] =
                  this._globalPostParams[globalPostParam];
              }
            }
          }
  ...
  private _executeXmlHttprequestSend(
    ...
    try {
      ...
      throw new Error(
        "HTTPManager could not execute request to " + url + "\n" + String(e),
      );
    }
  }
  ...
  private _generateValidRequestsList(
    ...
    if (ArrayUtils.isArray(requests)) {
      ...
      if (
        StringUtils.isString(requests) &&
        !StringUtils.isEmpty(requests as string)
      ) {
  ...
  queue(
    ...
    for (let queue of this._queues) {
      if (queue.name === queueName) {
        ...
        if (finishedCallback !== null) {
          let dummyRequest = new HTTPManagerGetRequest(
            "FINISHED_REQUEST_CALLBACK",
          );
    ...
    throw new Error(
      `queue ${queueName} does not exist. Create it with createQueue()`,
    );
  }
  ...
  private _startQueue(name: string) {
    ...
    let runRequests = (queue: {
      ...
      if (queue.pendingRequests.length <= 0) {
        ...
        if (
          queue.pendingRequests[queue.pendingRequests.length - 1].url ===
          "FINISHED_REQUEST_CALLBACK"
        ) {
          let finallyCallback =
            queue.pendingRequests.pop() as HTTPManagerBaseRequest;
  ...
  loadResourcesFromList(
    ...
    if (
      !StringUtils.isString(urlToListOfResources) ||
      StringUtils.isEmpty(urlToListOfResources)
    ) {
    ...
    this.execute(urlToListOfResources, (results, _anyError) => {
      ...
      for (var resource of resourcesList) {
        resourcesFullUrls.push(
          StringUtils.formatPath(this._composeUrl(baseUrl, resource), "/"),
        );
      }
  ...
  private _composeUrl(baseUrl: string, relativeUrl: string) {
    ...
    if (
      ...
      composedUrl = StringUtils.replace(
        StringUtils.formatPath(baseUrl + "/" + relativeUrl, "/"),
        ["http:/", "https:/"],
        ["http://", "https://"],
        1,
      );
    ...
    if (
      this.isOnlyHttps &&
      composedUrl.substr(0, 5).toLowerCase() === "http:"
    ) {
      throw new Error(
        "Non secure http requests are forbidden. Set isOnlyHttps=false to allow " +
          composedUrl,
      );
    }
