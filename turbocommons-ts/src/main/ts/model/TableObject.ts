...
export class TableObject {
  ...
  constructor(rows = 0, columns: number | string[] = 0) {
    ...
    if (NumericUtils.isInteger(columns) && (columns as number) >= 0) {
      ...
      if (ArrayUtils.isArray(columns)) {
        ...
        throw new Error(
          "constructor columns must be an integer or an array of strings",
        );
      }
    }

    if (
      this._columnsCount + this._rowsCount > 0 &&
      (this._columnsCount == 0 || this._rowsCount == 0)
    ) {
      throw new Error(
        "constructor columns cannot be empty if rows are positive and vice versa",
      );
    }
  ...
  getColumnNames() {
    ...
    for (let i = 0; i < this._columnsCount; i++) {
      ...
      result.push(
        this._columnNames.isKey(key) ? this._columnNames.get(key) : "",
      );
    }
